...
func displayProfilePicture(view: UIView, minY: CGFloat) -> CGFloat {
    let profilePicture = UIImageView()
    profilePicture.frame = CGRect(x: 0, y: 0, width: DisplayUtility.screenWidth, height: 0.5622*DisplayUtility.screenHeight)
    if let data = localData.getMainProfilePicture() {
        ...
        profilePicture.image = UIImage(cgImage: newCGImage!)
