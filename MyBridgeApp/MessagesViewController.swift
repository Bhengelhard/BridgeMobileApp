        ...
        query.findObjectsInBackground(block: { (results, error) -> Void in
            ...
            if let error = error {
                ...
                for i in 0..<results.count{
                    ...
                    self.profilePicURLs[result.objectId!]?[result["user1_objectId"] as! String] =
                        result["user1_profile_picture_url"] as! String
                    self.profilePicURLs[result.objectId!]?[result["user2_objectId"] as! String] =
                        result["user2_profile_picture_url"] as! String
                }
            }
            ...
            if let error = error {
                ...
                for i in 0..<results.count{
                    ...
                    if user != "" && result["\(user)_response"] as! Int != 1 {
                        ...
                        downloader.imageFromURL(URL: profilePicURL!, imageView: profilePicView)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        for id in profilePicsDict.keys {
            if id != PFUser.current()?.objectId {
                ...
                let downloader = Downloader()
                downloader.imageFromURL(URL: url, imageView: cell.profilePic)
                break
            }
        }
